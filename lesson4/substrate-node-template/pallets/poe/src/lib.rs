...
pub trait Trait: system::Trait {
	...
	type MaxCommentLength: Get<u32>; //备注格式
}
...
decl_storage! {
	...
	trait Store for Module<T: Trait> as TemplateModule {
		...
		ProofsWithComments get(fn proofs_with_comments): map hasher(blake2_128_concat) Vec<u8> => (T::AccountId, T::BlockNumber, Vec<u8>);
	}
}
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		//创建
		#[weight = 0]
		pub fn create_claim_comment(origin, claim: Vec<u8>,comments: Vec<u8> ) -> dispatch::DispatchResult {
			let sender = ensure_signed(origin)?;
			...
			ProofsWithComments::<T>::insert(&claim,(sender.clone(), system::Module::<T>::block_number(),&comments)); //增加备注
